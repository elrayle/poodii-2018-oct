class Bowling
  attr_reader :rolls
  
  TRIGGERING_VALUE = 10
  STRIKE_TRIGGERING_ROLLS = 1
  STRIKE_BONUS_ROLL_COUNT = 2
  SPARE_TRIGGERING_ROLLS = 2

  def initialize(rolls)
    @rolls = rolls
  end

  def score
    running_score = 0
    current_frame = 0
    max_frames    = 10
    @remaining_rolls = rolls

    while current_frame < max_frames
      current_frame += 1

      
      # New Requirement: No-tap,
      #   where a strike/spare requires fewer than 10 pins.
      #
      # So, the 10's need to vary.
      # The sets of code below are identical _except_ for the 'magic' numbers.
      # If the numbers had names, you express every rule with common code like:
      #
      # if (remaining_rolls.take(_num_triggering_rolls).sum) >= _triggering_value
      #   if remaining_rolls.size >= _num_rolls_to_score
      #     running_score += remaining_rolls.take(_num_rolls_to_score).sum
      #     remaining_rolls = remaining_rolls.drop(_num_triggering_rolls)
      #   end
      #   next
      # end

      # strike
      frame_score = score_frame(10, 1, 3)
      if frame_score.positive?
        running_score += frame_score
        next
      end

      # if (remaining_rolls.take(1).sum) == TRIGGERING_VALUE
      #   if remaining_rolls.size > 2
      #     running_score += remaining_rolls.take(3).sum
      #     remaining_rolls = remaining_rolls.drop(1)
      #   end
      #   next
      # end

      # spare
      frame_score = score_frame(10, 2, 3)
      if frame_score.positive?
        running_score += frame_score
        next
      end


      # if (remaining_rolls.take(2).sum) == TRIGGERING_VALUE
      #   if remaining_rolls.size > 2
      #     running_score  += remaining_rolls.take(3).sum
      #     remaining_rolls = remaining_rolls.drop(2)
      #   end
      #   next
      # end

      # open frame
      running_score += score_frame(0, 2, 2)

      # if (remaining_rolls.take(2).sum) >= 0
      #   if remaining_rolls.size >= 2
      #   running_score += remaining_rolls.take(2).sum
      #   remaining_rolls = remaining_rolls.drop(2)
      #   end
      #   next
      # end
    end

    running_score
  end

  def score_frame(triggering_value, num_triggering_rolls, num_rolls_to_score)
    if (@remaining_rolls.take(num_triggering_rolls).sum) >= triggering_value
      if @remaining_rolls.size > num_rolls_to_score
        frame_score = @remaining_rolls.take(num_rolls_to_score).sum
        remaining_rolls = @remaining_rolls.drop(num_triggering_rolls)
      end
      return frame_score
    else
      return -1
    end

  end
end