...
class StandardRollParserTest < Minitest::Test
  ...
  def frame_configs
    [
      ...
      {num_triggering_rolls: 2, triggering_value:  8, num_rolls_to_score: 4},
  ...
  def test_parse_all_gutter_balls
    rolls = [0] * 20
    assert_equal [2, 2, [0, 0]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end

  def test_all_ones
    rolls = [1] * 20
    assert_equal [2, 2, [1, 1]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end

  def test_one_spare
    rolls = [5, 5, 3] + [0] * 17
    assert_equal [2, 3, [5, 5, 3]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end

  def test_one_strike
    rolls = [10, 3, 4] + [0] * 18
    assert_equal [1, 3, [10, 3, 4]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end

  def test_perfect_game
    rolls = [10] * 12
    assert_equal [1, 3, [10, 10, 10]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end

  def test_magic_eight
    rolls = [7, 1, 4, 5, 6] * 12
    assert_equal [2, 4, [7, 1, 4, 5]], StandardRollParser.new.parse(rolls: rolls, frame_configs: frame_configs)
  end
